...
export default function ProductForm({ product, locale = "en" }) {
  ...
  // Reorder photos by drag-and-drop; the first photo stays the main image.
  ...
  // Upload one file: signature from our admin-only endpoint, then straight to
  // Cloudinary. Returns a browser-friendly (f_auto) URL.
  ...
  // at a time). Order is preserved so the first stays the main photo.
  async function uploadFiles(files) {
    ...
    try {
      ...
      if (urls.length) setImagesText((prev) => (prev ? `${prev}\n${urls.join("\n")}` : urls.join("\n")));
      if (failed) setUploadError(`${failed} ${failed === 1 ? "photo" : "photos"} failed to upload — try again`);
    } finally {
      setUploading(false);
  ...
  return (
    <form action={formAction} className="max-w-3xl space-y-8">
      ...
      <section>
        ...
        {imageUrls.length > 0 && (
          <>
            {imageUrls.length > 1 && <p className="mt-3 text-xs text-muted">{t.dragToReorder}</p>}
            <div className="mt-2 grid grid-cols-3 gap-3 sm:grid-cols-5">
              {imageUrls.map((url, i) => (
                <div
                  ...
                  className={`group relative aspect-[3/4] cursor-move overflow-hidden rounded-lg border bg-surface ${
